...
//   }
  array_move = (arr, old_index, new_index)=> {
   ...
   _updateColor = (ev, colorObj) => {
    setTimeout(() =>{
    const { index, dropelement }= this.state;
    for (let i = 0; i<dropelement.length; i++){ 
      if(i === index){
        if(dropelement[i].Name ==='ColorPicker'){
          dropelement[i].color = colorObj;
          this.setState({ dropelement });
         // console.log(dropelement[i].color);
        }
      } 
    }
    }, 120)
  ...
  _onFilterChanged = (filterText, tagList)=> {
    ...
    for(var i=0; i< dropelement.length; i++){
      if(dropelement[i].Name ==='Pickers'){
        _testTags = dropelement[i].items.map(item => ({ key: item, name: item }));
      }
    }
    return filterText
      ? _testTags
  ...
  componentDidMount()
  {
    let dropeelm= sessionStorage.getItem("dropelements");
    if(dropeelm){
      this.setState({
  ...
  render() { 
    ...
    sessionStorage.setItem("dropelement", JSON.stringify(dropelement));
    if(dropelement.length !== 0)
    {
       React.cloneElement(<Sample />, {dropelement: dropelement})
    }
        ...
        return <div className="form-group"  key={i+elem.Name} id={elem.Name + i } draggable onDragStart={(e)=>this.onDragStart(e, elem, i)} onDrop={(e)=>{this.onDrop(e, i)}} onDragLeave={(e)=> {this.dragLeave(e)}} onDragEnter={(e)=>{this.dragEnter(e)}}>
          ...
          menuProps={{
            ...
            items:elem.options.map((options, k)=> {
              return { key: options.Text, text: options.value}
          })
    ...
    <Icon iconName="BoxSubtractSolid" className="ms-IconExample deletebtn"  onClick={() => this.deletecolm(i)}/>
    <div className="field" onClick={() => this.showsettings(elem, i)}>
      <Text key={ elem.Name+ 'text' + i} variant={elem.Name} nowrap block>
        {elem.Text}
        </Text>
    </div></div>;
    case 'pickers':
    ...
    <ColorPicker color={elem.color} onChange={this._updateColor} alphaSliderHidden={elem.alphaSliderHidden} />
    
        <div className={classNames.column2}>
          <div
            className={classNames.colorSquare}
            style={{
              backgroundColor: elem.color.str
            }}
          />
         {/* <Toggle label="Hide alpha slider" onChange={this._onHideAlphaClick} checked={elem.alphaSliderHidden} /> */}
        </div>
      </div>
    </div></div>;
    case 'stack':
    ...
    return (
      <div className="DragDrop">
        <header className="dark"><h3>Drag And Drop Example</h3>
          <div className="previewdiv">  
            <PrimaryButton
              className="prevbtn"
              data-automation-id="test"
              text={<Link className="preview" href="/sample">Preview</Link>}
  ...
  _cancleclick = () =>{
    sessionStorage.removeItem('dropelement');
    sessionStorage.removeItem('dropelements');
    this.setState({
      dropelement: [],
      dropelementsettings: [],
      showResults: false
